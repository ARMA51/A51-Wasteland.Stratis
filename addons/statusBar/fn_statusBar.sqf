...
[] spawn {
		...
		switch(_minutes) do
	{
		...
		((uiNamespace getVariable "osefStatusBar")displayCtrl 1000)ctrlSetText format["																																																																																																										FPS: %1 | PLAYERS: %2 | RESTART: %3:%4", round diag_fps, count playableUnits, _hours, _minutes, _counter];
	}; 
};